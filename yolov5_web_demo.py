...
def save_video(filename):
    ...
    return jsonify({"success": True, "output_video": f"/outputs/{output_filename}", "frames_processed": frame_count})
...
def upload_coco_file():
    ...
    try:
        ...
        return jsonify({"error": f"Lỗi: {e!s}"}), 400
...
def start_training():
    ...
    def run_training():
        try:
            logger.log("🚀 Bắt đầu training với các than số:")
            ...
            for epoch in range(epochs):
                ...
                for i, img_file in enumerate(image_files):
                    ...
                    try:
                        ...
                        logger.log(f"⚠️ Lỗi khi xử lý {img_file}: {e!s}")
    ...
    return jsonify({"success": True, "message": "Training đã bắt đầu", "training_id": logger.training_name})
    ...
    return jsonify(
        {
            ...
            "training_id": training_state["current_training"].training_name
            if training_state["current_training"]
            else None,
            "summary": training_state["current_training"].get_summary_dict()
            if training_state["current_training"]
            else None,
        }
    )
...
def get_training_logs(training_id):
    ...
    try:
        ...
        if low_det_file.exists():
            with open(low_det_file, encoding="utf-8") as f:
        ...
        if low_det_dir.exists():
            low_det_images = [
                f"/training_logs/{training_id}/low_detection_images/{f.name}"
                for f in low_det_dir.glob("*")
                if f.is_file()
            ]
        ...
        if fp_dir.exists():
            fp_images = [
                f"/training_logs/{training_id}/false_positives/{f.name}" for f in fp_dir.glob("*") if f.is_file()
            ]
