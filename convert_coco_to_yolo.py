...
def convert_coco_to_yolo(coco_json_path, images_dir, output_labels_dir, copy_images_to=None):
    ...
    for img in coco_data["images"]:
        images_dict[img["id"]] = {"file_name": img["file_name"], "width": img["width"], "height": img["height"]}
    ...
    for ann in coco_data["annotations"]:
        ...
        category_id = ann.get("category_id", next(iter(categories_dict.keys())) if categories_dict else 1)
        ...
        annotations_by_image[image_id].append(
            {"class": class_idx, "x": x_center_norm, "y": y_center_norm, "w": w_norm, "h": h_norm}
        )
    ...
    return {
        ...
        "categories": {
            idx: name for cat_id, name in cat_id_to_name.items() for idx, _ in [(categories_dict[cat_id], None)]
        },
    }
...
if __name__ == "__main__":
    ...
    result = convert_coco_to_yolo(
        str(COCO_JSON), str(IMAGES_DIR), str(OUTPUT_LABELS), copy_images_to=str(OUTPUT_IMAGES)
    )
